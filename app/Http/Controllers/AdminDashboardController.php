...
class AdminDashboardController extends Controller
{
    ...
    //     // Ganti ekstensi file menjadi .xls agar Microsoft Excel langsung mengenalinya sebagai spreadsheet
    //     $fileName = 'Rekap_Presensi_' . str_replace(' ', '_', $event->nama_event) . '_' . Carbon::now()->format('Ymd_His') . '.xls';
    ...
    //     // Buat folder publik sementara untuk menyimpan file gambar fisik hasil ekspor
    //     $tempDir = public_path('exports_temp');
    //     if (!file_exists($tempDir)) {
    //         mkdir($tempDir, 0777, true);
    //     }

    //     // Garbage Collector: Hapus file sementara yang usianya lebih dari 5 menit untuk menghemat penyimpanan server lokal
    //     $files = glob($tempDir . '/*');
    //     foreach ($files as $file) {
    //         if (is_file($file) && (time() - filemtime($file) > 300)) {
    //             @unlink($file);
    ...
    //     // Helper closures untuk mengubah data Base64 menjadi berkas gambar fisik murni
    //     $convertBase64ToUrl = function($base64String, $prefix) use ($tempDir) {
    //         if (strpos($base64String, 'data:image') !== 0) {
    //             return $base64String; // Jika data sudah berupa URL biasa, langsung kembalikan
    //         }

    //         // Memisahkan metadata base64 dan payload datanya
    //         @list($type, $file_data) = explode(';', $base64String);
    //         @list(, $file_data) = explode(',', $file_data);

    //         // Menentukan ekstensi file berdasarkan tipe mime
    //         $extension = 'png';
    //         if (strpos($type, 'image/jpeg') !== false) {
    //             $extension = 'jpg';
    //         }
    ...
    //         // Decode string base64 dan simpan menjadi file gambar sesungguhnya
    //         file_put_contents($filePath, base64_decode($file_data));

    //         // Menghasilkan URL publik absolut (contoh: http://127.0.0.1:8000/exports_temp/...) agar terbaca Excel
    //         return url('exports_temp/' . $imageName);
    //     };

    //     // Buat stream download yang memuat tabel HTML khusus Microsoft Excel
    //     $callback = function() use ($presences, $event, $convertBase64ToUrl) {
    //         $file = fopen('php://output', 'w');
            
    //         // Output template HTML yang kompatibel dengan rendering tabel Microsoft Excel
    //         // Menentukan lebar kolom & tinggi baris absolut di style CSS khusus Excel
    //         fwrite($file, '
    //         <html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40">
    //         <head>
    //             <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    //             <!-- Style CSS untuk mengatur tata letak sel dan ukuran gambar agar terkunci di sel Excel -->
    //             <style>
    //                 table { border-collapse: collapse; table-layout: fixed; }
    //                 th, td { border: 1px solid #cbd5e1; padding: 10px; text-align: left; vertical-align: middle; font-family: sans-serif; font-size: 11px; }
    //                 th { background-color: #1e3a8a; color: #ffffff; font-weight: bold; }
    //                 .img-cell { text-align: center; vertical-align: middle; }
    //                 .nip-wa { mso-number-format:"\@"; } /* Kode khusus Excel agar mendeteksi teks mentah (mencegah angka ilmiah) */
    //             </style>
    //         </head>
    //         <body>
    //             <h2 style="font-family: sans-serif; color: #1e3a8a; margin-bottom: 2px;">REKAPITULASI PRESENSI KEHADIRAN</h2>
    //             <h4 style="font-family: sans-serif; color: #475569; margin-top: 0; margin-bottom: 20px;">Kegiatan: ' . htmlspecialchars($event->nama_event) . ' (' . Carbon::parse($event->tanggal_event)->translatedFormat('d F Y') . ')</h4>
    ...
    //             // Konversikan data Base64 ke file fisik publik dan ambil URL absolutnya
    //             $ttdUrl = $convertBase64ToUrl($pres->tanda_tangan, 'ttd');
    //             $fotoUrl = $convertBase64ToUrl($pres->foto_capture, 'foto');

    //             // Masukkan baris data ke dalam file stream HTML-Excel
    //             // PENTING: Kita atur tinggi baris (height: 80px;) dan batas dimensi gambar secara inline di <img width="..." height="...">
    //             // Ini memaksa engine Excel desktop memotong, menyusutkan, dan mengunci floating drawing ke koordinat sel tersebut.
    //             fwrite($file, '
    //                     <tr style="height: 80px;">
    //                         <td style="height: 80px;">' . ($idx + 1) . '</td>
    //                         <td style="height: 80px;">' . htmlspecialchars($tanggal) . '</td>
