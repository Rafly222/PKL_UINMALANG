...
class AuthController extends Controller
{
        ...
        if (Auth::attempt($credentials)) {
            ...
            if ($user->status !== 'approved') {
                ...
                if ($user->status === 'pending') {
                    return back()->with('warning', 'Pendaftaran akun Anda masih menunggu persetujuan (approval) dari Super Admin. Silakan hubungi admin via WhatsApp/Telepon ke nomor ' . config('app.support_phone') . ' untuk konfirmasi persetujuan akun Anda.');
                } else {
            ...
            $isBlacklisted = false;
            if ($user->nip) {
                $isBlacklisted = Blacklist::where('nip', $user->nip)->exists();
            }
            ...
            if ($isBlacklisted) {
                ...
                return back()->with('warning', 'Akses ditolak: Akun Anda ditangguhkan karena identitas NIP masuk dalam daftar Blacklist.');
            }
 
            $request->session()->regenerate();
 
            // Catat Log Aktivitas Sukses
            ActivityLog::log('login', "Pengguna '{$user->name}' (Role: {$user->role}) berhasil masuk ke dashboard.");
    ...
    public function handleRegister(Request $request)
    {
        ...
        ActivityLog::create([
            ...
            'user_name' => $user->name,
            'activity' => 'register',
            'description' => "Pendaftaran akun baru oleh '{$user->name}' (Email: {$user->email}).",
            'ip_address' => $request->ip(),
            'user_agent' => $request->userAgent()
        ]);
 
        return redirect('/login')->with('info', 'Pendaftaran berhasil! Akun Anda sedang menunggu persetujuan (approval) dari Super Admin sebelum dapat digunakan. Silakan hubungi admin via WhatsApp/Telepon ke nomor ' . config('app.support_phone') . ' untuk konfirmasi persetujuan akun Anda.');
    }
 
    public function logout(Request $request)
    {
        if (Auth::check()) {
            ActivityLog::log('logout', "Pengguna '" . Auth::user()->name . "' keluar dari dashboard.");
        }
