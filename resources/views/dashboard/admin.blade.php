    ...
    <div class="modal-content border-0 ep-card">
      ...
      <div class="modal-body text-start p-4">
        <form action="{{ route('admin.event.store') }}" method="POST">
          ...
          <div class="mb-3" id="admin-event-password-group" style="display: none;">
            <label class="form-control-label text-xs font-weight-bold">Password Event Privat</label>
            <div class="input-group">
              <input type="text" name="password" id="admin-event-password" class="form-control" placeholder="Password event privat" style="border-top-right-radius: 0; border-bottom-right-radius: 0;">
              <button type="button" class="btn bg-gradient-dark mb-0 px-3" id="btn-generate-create-admin-password" style="border-top-left-radius: 0; border-bottom-left-radius: 0; font-size: 11px;">
...
<script>
  ...
  const btnGenerateCreateAdminPassword = document.getElementById('btn-generate-create-admin-password');
  ...
  function toggleAdminPassword() {
    if (adminAccessType.value === 'privat') {
      adminEventPasswordGroup.style.display = 'block';
      if (!adminEventPasswordInput.value) {
        adminEventPasswordInput.value = generateRandomPassword();
      }
    } else {
      adminEventPasswordGroup.style.display = 'none';
    }
  }
  ...
  if (btnGenerateCreateAdminPassword && adminEventPasswordInput) {
    btnGenerateCreateAdminPassword.addEventListener('click', function() {
      adminEventPasswordInput.value = generateRandomPassword();
    });
  }
